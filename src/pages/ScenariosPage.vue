...
/* ── Choose dialog (for scenarios with mutually exclusive choices) ── */
...
const chooseOptions = computed(() => {
  ...
  return def.choices.map((id) => ({
    ...
    status: scenarioStore.getScenarioStatus(id),
  }))
})
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-0.5">
              ...
              >
                ...
                1 z {{ scenarioStore.getChoiceGroup(s.id)?.total }}
              </span>
              <span
            ...
            <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4 modal-scroll">
              ...
              <div v-if="selectedScenario.choices?.length && selectedScenario.choices.length > 1">
                <h3 class="text-xs font-semibold uppercase tracking-wider mb-2 flex items-center gap-1.5"
                  ...
                  {{ selectedScenario.computedStatus === SCENARIO_STATUSES.COMPLETED ? 'Volba scénáře' : 'Rozcestí — vyber 1 z ' + selectedScenario.choices.length }}
                </h3>
                <p v-if="selectedScenario.computedStatus !== SCENARIO_STATUSES.COMPLETED" class="text-xs text-amber-400/70 mb-2">
                  Po dokončení tohoto scénáře odemkneš pouze jeden z následujících:
                </p>
                <div class="flex flex-wrap gap-1.5">
                  <button
          ...
          <div class="relative w-full max-w-sm bg-fh-card border border-amber-400/25 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-3 border-b border-amber-400/20">
              ...
              <p class="text-xs text-amber-400/70">
                Tento scénář má vzájemně výlučné pokračování. Vyber jeden — ostatní budou uzamčeny.
              </p>
            </div>
            <!-- options -->
            ...
            <div class="px-6 py-4 border-t border-fh-border flex justify-end gap-2">
              <button
                class="px-4 py-2 text-xs text-gray-400 hover:text-gray-300 transition-colors"
                @click="cancelChoice"
              >
                Zrušit
              </button>
